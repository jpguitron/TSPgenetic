    ...
    def getTotalCost(self):
        ...
        for i in range(len(self.nodes)):
            if i == 0:
                ...
                totalCost += self.nodes[i].getCost(self.nodes[len(self.nodes)-1])
            elif i == len(self.nodes)-1:
                totalCost += self.nodes[i].getCost(self.nodes[i-1])
                totalCost += self.nodes[i].getCost(self.nodes[0])
            else:
                totalCost += self.nodes[i].getCost(self.nodes[i-1])
                totalCost += self.nodes[i].getCost(self.nodes[i+1]) 
        for i in range(len(self.nodes)):
            if i+1 < len(self.nodes):
                totalCost += (self.nodes[i].getCost(self.nodes[i+1])*3) 
            else:
                totalCost += (self.nodes[0].getCost(self.nodes[len(self.nodes)-1])*3)
        return totalCost
